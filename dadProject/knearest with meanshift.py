...
##    dist[i] = np.mean(dist[i])
##    dist[i] = np.std(dist[i])
    dist[i] = np.percentile(dist[i],90)
...
'''
for i in range(len(clf.cluster_centers_)):
    size = (dist[i]+num_label[i])*40
##    size = dist[i]*5000
##    size = dist[i]*500
    plt.scatter(clf.cluster_centers_[i][0], clf.cluster_centers_[i][1], facecolors='none', edgecolors='r', s=size)
'''
