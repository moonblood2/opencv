...
##    tf_train_labels = tf.constant(train_labels[:train_subset])
    ...
    weights1 = tf.Variable(tf.truncated_normal([28*28,num_hidden_nodes]))
    biases1 = tf.Variable(tf.zeros([num_hidden_nodes]))

    weights2 = tf.Variable(tf.truncated_normal([num_hidden_nodes,10]))
    biases2 = tf.Variable(tf.zeros([10]))

    lay1_train = tf.nn.relu6(tf.matmul(tf_train_dataset,weights1)+biases1)
...
##########################################################
    ...
    lay1_valid = tf.nn.relu6(tf.matmul(tf_valid_dataset, weights1) + biases1)
    valid_prediction = tf.nn.softmax(tf.matmul(lay1_valid, weights2) + biases2)
    lay1_test = tf.nn.relu6(tf.matmul(tf_test_dataset, weights1) + biases1)
